...
- [x] **UI-13**: User salva estratégia (só salvar) ou salva-e-executa, com selo "filtro garantido pelo Core" e status das fontes visível — done 07-02 (SearchForm §6 + search-form com saúde via listSources/getSourceHealth + Idempotency-Key por toque) + 08-09 (decisões Paulo 12/09 DEFINITIVAS: checkbox mestrado profissional ↔ professionalMaster, termo cru sem auto-aspas)
- [x] **UI-14**: User executa busca e acompanha o run com progresso por fonte (cancelável) até ok / parcial (banner âmbar com o que faltou) / falha (motivo + repetir) / cancelled — done 07-03 (useRunPolling 2500ms/teto 240 + run.tsx §7 + tripwire 6/6) + busca COMPLETA 08-06 (decisão Paulo 12/09: loop 50/pág até o total, timeout 30min, UI 720 polls + página X de ~Y + cancela sempre; prova 120/120 + rerun 0 + cancel 50/cancelled) + lote INCREMENTAL 08-07 (decisão Paulo 12/09 REVISADA, SUPERSEDE 08-06: run 1 lote 100/fonte + totalKnown, BUSCAR MAIS +100/fonte com newCount/lote, UI botão/loading/retry + X de Y, polling 240; prova 250 em 100+100+50 + IDOR) + gap CAPES 08-08 (auditoria Hermes 12/09: payload sem combo Ano+Grau + year de dataDefesa; período+tipo volta a retornar) + decisões definitivas 08-09 (MP terceiro valor separado, payload Ano-only com ambos, sem Grande Área, hint 2026)
...
| UI-13 | Phase 7 | Done (07-02 + checkbox MP/termo cru 08-09) |
| UI-14 | Phase 7 + 8 | Done (07-03 + busca completa 08-06 SUPERSEDED + lote incremental 08-07 + gap CAPES Ano+Grau 08-08 + decisões definitivas Ano-only/MP 08-09) |
